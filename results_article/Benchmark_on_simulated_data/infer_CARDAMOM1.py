...
for r in range(N):
    ...
    model.fit(x)
    score = model.inter
    np.save('CN5/CARDAMOM1/score_{}'.format(r+1), score)
...
for r in range(N):
    ...
    model.fit(x, verb=1)
    score = model.inter
    np.save('FN4/CARDAMOM1/score_{}'.format(r+1), score)
...
for r in range(N):
    ...
    model.fit(x)
    score = model.inter
    np.save('FN8/CARDAMOM1/score_{}'.format(r+1), score)
...
for r in range(N):
    ...
    model.fit(x)
    score = model.inter
    np.save('BN8/CARDAMOM1/score_{}'.format(r+1), score)
...
for n in [5,10,20]:
    ...
    for r in range(N):
        ...
        runtime[r] = t1 - t0
        score = model.inter
        np.save('Trees{}/CARDAMOM1/score_{}'.format(n,r+1), score)
    # Save running times
    np.savetxt('Trees{}/CARDAMOM1/runtime.txt'.format(n), runtime.T)
