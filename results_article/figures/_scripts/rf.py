...
#Code slightly adapted from https://github.com/zsteve/referencefitting/tree/main
...
class Estimator:
    def __init__(self, adatas, kos, weights = None, t_key = "t", pca = "separate", norm = True, scale = None, A0 = None, b0 = None, drift = False, lr = 1e-2, iter = 1000, ot_coupling = True, n_pca_components = 10, 
        ...
        if self.norm:
            ...
            self.std = torch.tensor(np.sqrt(self.scaler.var_+0.0001)) #Added +10-e4 for stability
            # self.Xs = [[self.scaler.transform(adata.X[adata.obs.t == i, :]) for i in np.unique(adata.obs['t'])] for adata in adatas]
            self.Xs = [[adata.X[adata.obs[t_key] == i, :] - self.scaler.mean_ for i in np.unique(adata.obs[t_key])] for adata in adatas]
        else:
            self.scaler = None
            self.Xs = [[adata.X[adata.obs[t_key] == i, :] for i in np.unique(adata.obs[t_key])] for adata in adatas]
            self.std = torch.ones(adatas[0].X.shape[1], dtype = torch.float64)
        if self.pca == "common":
            ...
            self.M_pca = [torch.tensor(self.pca.components_.T, device = self.device, dtype = torch.float64) for _ in range(len(self.kos))]
        elif self.pca == "separate":
            self.pca = [sk.decomposition.PCA().fit(np.vstack(x)) for x in self.Xs]
            self.M_pca = [torch.tensor(_pca.components_.T, device = self.device, dtype = torch.float64) for _pca in self.pca]
        ...
        self.Xs = [[torch.tensor(x, dtype = torch.float64).to(self.device) for x in y] for y in self.Xs]
        # set up scale
        self.scale = scale
