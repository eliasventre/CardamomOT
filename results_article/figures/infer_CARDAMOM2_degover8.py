...
for r in range(0, N):
    ...
    np.save('FN4/CARDAMOM2_degover8/score_{}'.format(r+1), score)
...
for r in range(0, N):
    ...
    model.fit(x, verb=verb)
    score = model.inter_t[-1]
    np.save('CN5/CARDAMOM2_degover8/score_{}'.format(r+1), score)
...
for r in range(0, N):
    ...
    model.fit(x, verb=verb)
    score = model.inter_t[-1]
    np.save('BN8/CARDAMOM2_degover8/score_{}'.format(r+1), score)
...
for r in range(0, N):
    ...
    model.fit(x, verb=verb)
    score = model.inter_t[-1]
    np.save('FN8/CARDAMOM2_degover8/score_{}'.format(r+1), score)
