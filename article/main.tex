...
1 - COMPO, Inria Méditerranée, Cancer Research Center of Marseille, Inserm UMR1068, CNRS UMR7258, UM105, Aix-Marseille Université, SouthROCK Center of Excellence in Pediatric Oncology Research, 13273 Marseille, France.
\par
\textbf{contact}: elias.ventre@inria.fr.
...
\vspace{.5cm}
\noindent \textbf{Keywords}: gene \firstreview{regulatory} network, mechanistic learning, transcriptional bursting, trajectory inference, optimal transport, generative modeling.\par
\newpage
...
- d_0\,M(t),
\end{equation}
which we use for evaluation of trajectory inference in Figure \ref{figure_S7}.
...
On all benchmark networks, CardamomOT achieved consistently higher AUPR scores than all competing methods (Figure~\ref{figure3}A). The improvement was particularly pronounced for the more complex networks (FN8, BN8, and trees with more than 10 genes), where CardamomOT's mechanistic modeling and trajectory inference provided a substantial advantage over methods that rely solely on correlation-based or quasi-static approximations. CardamomOT infers both the sign and direction of regulatory interactions, which is crucial for understanding causal relationships in GRNs. When comparing against methods that only infer undirected edges (Figure~\ref{figure3}B), CardamomOT maintained its performance advantage, highlighting the value of mechanistic modeling for resolving regulatory directionality.

The convergence of the inference procedure is illustrated in Figure~\ref{figure_S8}: 
...
Moreover, the runtimes reported in Table \ref{tableS2} show that CardamomOT scales 
...
rate $d_1 = \ln 2 / t_{1/2}$, since the two are inversely related. To evaluate robustness to uncertainty in these rates, we ran CardamomOT on simulated data while systematically perturbing the input degradation rates from $\pm 20\%$ up to $\pm 400\%$ of their true values. The method maintained high AUPR scores (averaged across FN4, FN8, BN8 and CN5) even with moderate perturbations ($\pm 50\%$), and clearly outperformed random guessing and competing benchmark methods even with very large perturbations ($\pm 200$--$400\%$, Figure~\ref{figure4}F).
Typical literature-derived
estimates therefore fall around the upper end of the range in which CardamomOT retains high
accuracy (Figure~\ref{figure4}F), while short-lived proteins and the tail of the uncertainty
...
scRNAseq measurements are prone to technical noise due to inefficient capture and sequencing. To test CardamomOT robustness to technical noise, we performed inference on simulated datasets with increasing levels of binomial dropout (5\% -- 90\%, averaged across FN4, FN8, BN8 and CN5). CardamomOT maintains high AUPR scores with slightly increasing variability at high levels of dropout, confirming robustness to moderate technical noise (Figure~\ref{figure4}G). 
}

\firstreview{
\paragraph{CardamomOT is robust to temporal variability in measurements.}
scRNAseq experiments commonly show irregular measurement schedules. We thus tested CardamomOT robustness to variation in measurement times by performing inference on simulated datasets on a scale going from equally spaced timepoints to randomly sampled timepoints. Average AUPR across FN4, FN8, BN8 and CN5 only slightly degrades highly variable scales, confirming CardamomOT capability to work with different measurement schedules (Figure~\ref{figure4}H). 
}

Overall, \firstreview{these results establish CardamomOT as a robust method for GRN inference on the benchmark considered.}

\firstreview{\paragraph{Interpreting the ordering of methods in Figure~\ref{figure3}A}
An important detail in Figure~\ref{figure3}A deserves comment: CARDAMOM, which does not
...
The ablation study of Figure 4A–E supports this reading directly. The \texttt{noloop} variant returns the network obtained after a single round of the procedure, without the iterative refinement of Steps 1–3, performing a single round of OT on the burst modes. Its accuracy is accordingly lower than that of the full method — substantially so on BN8, yet it remains above RF on all four benchmark networks (compare Figure \ref{figure4}A–D with Figure \ref{figure3}A).
...
(Figures~\ref{figure_S1}--\ref{figure_S3}). The gene sets used for each dataset, and the virtual
...
(Figure~\ref{figure_S9}), indicating reproducible overall network structure with residual
run-to-run variability.} The inferred GRNs are notably sparse
(Figures~\ref{figure_S1}A--\ref{figure_S3}A), with a small number of hub regulators
...
regulatory strength are reported with their main targets in Table~\ref{tableS3}.} The most
connected regulators are consistent with those identified in the original studies: Hoxb2,
Dnmt3a, Lamb1 and Col4a2 for Semrau (Figure~\ref{figure_S1}C), in agreement with \cite{Semrau_2017},
and CHGA among the dominant regulators for Kameneva (Figure~\ref{figure_S2}C), consistent with
\cite{gandrillon2025inferring}. \firstreview{Several highly expressed housekeeping genes (RPL30,
ATP5F1E) also rank among the top Kameneva regulators (Table~\ref{tableS3}), which likely
...
(Figures~\ref{figure_S1}B--\ref{figure_S3}B). While often excluded in GRN inference frameworks
...
data (Figure~\ref{figure5}), evaluated in both transcript and protein space;
    \item the accuracy of the predicted \textbf{cell-to-cell
couplings} on true trajectory data when simulated cells are not virtually killed at each timepoint (Figure~\ref{figure_S7});
    \item the similarity of the hidden protein levels reconstructed by the method compared to the simulated ones, which are not used by the algorithm (Figure~\ref{figure_S6}).
\end{itemize}
...
(Figure~\ref{figure5}A). This reflects an intrinsic limitation: mRNA levels
...
(Figure~\ref{figure5}E), bearing little resemblance to the structured, GRN-driven dynamics
of the system. In contrast, CardamomOT achieves cosine similarities exceeding $0.8$
across all networks (Figure~\ref{figure5}A), and the corresponding UMAP stream plots
(Figure~\ref{figure5}C) show velocity fields with coherent, biologically interpretable
structure.
...
Figure~\ref{figure5}B), suggesting that even simplified linear models can partially capture
protein-level trends. The resulting velocity field is nevertheless still close to a uniform flow in UMAP space, and poorly reflects the true temporal evolution of cells (Figure~\ref{figure5}F). 
...
higher cosine similarities (again exceeding $0.75$, Figure~\ref{figure5}B), with
protein UMAP stream plots (Figure~\ref{figure5}D) showing well-structured trajectories
...
across consecutive timepoints (Figure~\ref{figure_S7}). In this
...
CardamomOT achieves consistently lower descendant distances than RF on mRNA (Figure~\ref{figure_S7}A) and proteins (Figure~\ref{figure_S7}B), confirming that the mechanistic coupling provides more accurate cell-to-cell correspondences. 
...
reconstructed protein trajectories to the true simulated ones across all benchmark networks (Figure~\ref{figure_S6}). The inferred protein UMAP embeddings 
...
extensively in joint mRNA UMAP space (Figure~\ref{figure6}A--F), confirming that the 
...
exhibiting realistic stochastic variability. \firstreview{This analysis was repeated using the trajectory-preserving method PHATE \cite{Moon2019}, providing qualitatively consistent visualization of the global trajectory structure (Figure~\ref{figure_S10}).}
...
with those of the NB mixture model at all timepoints (Figures~\ref{figure_S4} and 
\ref{figure_S5}; $>0.95$ correlation of gene pair correlations across all datasets), 
...
strongest correlations observed in Figure~\ref{figure_S5} is thus inherited from 
...
and variance (Figure~\ref{figure6}J--O). This is a particularly stringent test: 
...
from the base simulated dataset shown in Figure~\ref{figure7}.
...
error bars in Figure~\ref{figure7}.
...
to the wild-type simulation (Figure~\ref{figure7}A). The predicted regulatory 
...
to control conditions (Figure~\ref{figure7}D,E), in line with the experimental 
...
including {Esrrb} and {Dppa4} (Figure~\ref{figure7}D), both well-known 
markers of the naive pluripotent state. {Obox6} OV similarly activates 
{Gata3}, {Lncenc1} and {Mycn}, and notably upregulates {Zfp42} 
itself (Figure~\ref{figure7}E), suggesting a positive feedback loop between the 
...
(Figure~\ref{figure_S11}): AUPR plateaus from $n_{\mathrm{pas}} \approx 4$ onwards on all four
...
Klf2 and Klf4, while activating Dnmt3a and Jarid2 (Figure \ref{figure_S1}F), 
...
the mesenchymal-to-epithelial transition (Figure~\ref{figure_S3}E). The Serum 
...
(Figure~\ref{figure_S3}G). Together, these stimulus-specific regulatory programs 
...
The algorithm CardamomOT, as well as the codes for generating the figures of this article, are available on \url{https://github.com/eliasventre/CardamomOT}, and the website describing the package can be found at \url{https://cardamomot.readthedocs.io/en/latest/}.

\firstreview{Data analyzed in this article are available at :
\begin{itemize}
  \item Semrau et al.\ (2017): GEO accession GSE79578.
  \item Kameneva et al.\ (2021): GEO accession GSE147821.
  \item Schiebinger et al.\ (2019): GEO accession GSE106340 and 
  \url{https://singlecell.broadinstitute.org/single_cell/study/SCP295/}
\end{itemize}
}
...
\firstreview{This work was supported
...
%\bibliography{proj_lib}

\begin{figure}
\centering
\fbox{\includegraphics[width=.8\textwidth]{figure_1.eps}}
    \caption{
